...
/**
 * Load the repository's single `.env`, which Next would otherwise never see.
 *
 ...
 * documents, `make harness` validates, and the gateway and worker both read. The result
 * was a web app that compiled, type-checked, tested and then answered 500 with
 * "Missing publishableKey", for as long as Clerk had been configured.
 *
 ...
 * dependency for twelve lines. The format is already constrained: `make harness` rejects
